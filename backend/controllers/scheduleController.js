...
const getUserSchedules = async (req, res) => {
  try {
    const userId = req.user.userId;
    const schedules = await Schedule.find({ user: userId }).sort({
      startTime: 1,
    });
  ...
  try {
    ...
    if (!updatedSchedule) {
      return response(
        res,
        404,
        "Lịch trình không tồn tại hoặc bạn không có quyền chỉnh sửa"
      );
    }
  ...
  try {
    ...
    if (!schedule) {
      return response(
        res,
        404,
        "Lịch trình không tồn tại hoặc bạn không có quyền xóa"
      );
    }
...
const getScheduleByIdUser = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Lỗi lấy lịch trình", error: error.message });
  }
};

const getNextSchedule = async (req, res) => {
  try {
    const userId = req.user.userId;
    const now = new Date();
    const nextEvent = await Schedule.findOne({
      user: userId,
      endTime: { $gte: now },
    }).sort({ startTime: 1 });
    if (!nextEvent) {
      return response(res, 200, "Không có lịch trình tiếp theo", null);
    }
    return response(res, 200, "Lấy lịch trình tiếp theo thành công", nextEvent);
  } catch (error) {
    console.log(error);
    return res
      .status(500)
